...
```sql function is_valid_json
DROP FUNCTION IF EXISTS is_valid_json_array;
CREATE FUNCTION is_valid_json_array(p_value text) returns boolean AS
            $$
            BEGIN
              return JSON_ARRAY_LENGTH(p_value::json) > 0;
            EXCEPTION
              WHEN others THEN
                 return false;
            END;
...
CREATE FUNCTION is_valid_json(p_value text) returns boolean AS
            $$
            BEGIN
              return (p_value::json IS NOT NULL);
            EXCEPTION
              WHEN others THEN
                 return false;
            END;
